document.addEventListener("DOMContentLoaded", () => {
	...
	function drawLines() {
		setTimeout(() => {
			...
			const allListItems =
				document.querySelectorAll(".hierarchy-list li");
			...
			document
				.querySelectorAll(".hierarchy-list li")
				.forEach((parentLi) => {
					...
					if (childUl && isExpanded) {
						...
						const parentRect =
							parentContent.getBoundingClientRect();
						...
						childLis.forEach((childLi, index) => {
							...
							path.setAttribute("stroke-width", ".72");
	...
	function toggleNode(event) {
		...
		if (targetLi && targetLi.classList.contains("folder")) {
			...
			// Handle file clicks - show dependency diagram
	...
	function analyzeDependencies(selectedFile) {
		// Get all file nodes from the hierarchy
		...
		// Mock dependency analysis (in real app, this would scan actual file contents)
	...
	function mockDependencyAnalysis(
		...
		// Create mock dependencies based on file patterns and relationships
		allFiles.forEach((fileNode) => {
			...
			// Mock import logic based on common patterns
			let isImported = false;
			let relationship = "direct";

			// Enhanced logic for JavaScript/TypeScript files
			if (
				...
				// Same directory files (higher probability for related files)
				...
				// Utility and library files from parent directories
				else if (
					(fileName.includes("util") ||
			...
			// Enhanced logic for special files (index, main, root, default, home)
			if (isSpecialFile(selectedFileName)) {
				// Index/main files are entry points - they import many things
				if (fileDir === selectedDir) {
					// High probability of importing files in same directory
					isImported = Math.random() > 0.3;
					relationship = "local-import";
				} else if (fileDir.startsWith(selectedDir + "/")) {
					// Import subdirectory index files
				...
				// Parent directory name matching
				const parentDirName = selectedDir.split("/").pop();
				if (
					parentDirName &&
					(fileName
						.toLowerCase()
						.includes(parentDirName.toLowerCase()) ||
			...
			// CSS/Style files might be imported by many JS files
			if (
		...
		// Sort dependencies by relationship importance
	...
	function createDependencyTree(container, data) {
		...
		const links = g
			...
			.style("stroke-width", (d) => {
				// Thicker lines for more important relationships
				...
				return importantRelationships.includes(
					d.target.data.relationship,
				)
					? "3px"
					: "2px";
			})
		...
		nodes
			...
			.style("fill", (d) => {
				if (d.depth === 0) return "#10b981"; // Green for root
				switch (d.data.relationship) {
					case "module-index":
					case "submodule":
						return "#8b5cf6"; // Purple for indexes
					case "directory-match":
					case "name-match":
						return "#06b6d4"; // Cyan for matches
					case "utility":
					case "shared-utility":
						return "#f59e0b"; // Yellow for utilities
					case "component":
					...
					case "configuration":
						return "#ef4444"; // Red for config
					case "type-definition":
						return "#a855f7"; // Purple for types
					case "local-import":
					case "local":
						return "#22c55e"; // Green for local
					case "child-module":
						return "#84cc16"; // Lime for children
					default:
		...
		const linkLabels = g
			...
			.style("opacity", 0.8)
			.text((d) => {
				// Shortened relationship labels for better readability
				const labelMap = {
					"module-index": "index",
				...
				return (
					labelMap[d.target.data.relationship] ||
					d.target.data.relationship
				);
			});
	...
	document
		.getElementById("dependency-modal")
